...
def fetch_data(query):
    ...
    if ct:
        try:
            ...
            cr.execute(query)
            data=cr.fetchall()
            colu=[i[0] for i in cr.description]
...
menu = st.sidebar.selectbox("Manu",["Home", "Data Analytics & visuals", "Predict Logs"])  
...
# Data Analytics and Visuals
    ...
    # SQL Query to fetch violation and its counts
        ...
        query="select violation,count(violation) as counts from secure.check group by violation"
        data=fetch_data(query)
        st.dataframe(data)
    ...
    # SQL Query to fetch Gender of Driver and its counts
         ...
         query="select driver_gender, count(*) as count from secure.check group by driver_gender"
         data=fetch_data(query)
         st.dataframe(data)
...
# Questions are added into selectbox
    ...
    query_map={
            "Top 10 vehicle_number involved in drug_related stops": "select vehicle_number from secure.check where drugs_related_stop=1 order by vehicle_number desc limit 10",
            "Most frequently searched vehicle":"select vehicle_number, count(*) as count from secure.check group by vehicle_number order by vehicle_number desc limit 1",
            "Driver age group had the highest arrest rate" : """select case
                                                            ...
                                                            from secure.check group by age_group
            ...
            "Driver Violation Trends Based on Age and Race" :  """select driver_age,driver_race,violation,count(*) as total_violation
                                                                  ...
                                                                  (select case
                                                                   ...
                                                                   FROM secure.check) AS grouped_data
