...
def db_connection():
    try:
        my_db = mysql.connector.connect(
            host="localhost",
            user="root",
            password="Kanishkaran12@",
            database="SECURE_LIST"
        )
        return my_db
...
menu = st.sidebar.selectbox(
    "Menu",
    ["Home", "Data Analytics & Visuals", "View Logs", "Predict Logs"]
)
...
if menu=='Home':    
    ...
    query="select * from SECURE_LIST_LOG"
    data=fetch_data(query)
    st.dataframe(data, use_container_width=False, width="content")
    st.markdown("---")  
    ...
    # SQL Query to fetch violation and its counts
        ...
        query="select violation,count(violation) as counts from SECURE_LIST_LOG group by violation"
        data=fetch_data(query)
        st.dataframe(data)
    ...
    # SQL Query to fetch Gender of Driver and its counts
         ...
         query="select driver_gender, count(*) as count from SECURE_LIST_LOG group by driver_gender"
         data=fetch_data(query)
         st.dataframe(data)
...
# View Logs
    ...
    "Stop duration distribution by violation"   ,
    ...
    query_map={
            "Top 10 vehicle_number involved in drug_related stops": "select vehicle_number from SECURE_LIST_LOG where drugs_related_stop=1 order by vehicle_number desc limit 10",
            "Most frequently searched vehicle":"select vehicle_number, count(*) as count from SECURE_LIST_LOG group by vehicle_number order by count desc limit 1",
            "Driver age group had the highest arrest rate" : """select case
                                                            ...
                                                            from SECURE_LIST_LOG group by age_group
            ...
            "Time of day sees the most traffic stops" : """select extract(hour from stop_time) as hour_of_day, count(*) as total_stops
                                                                        ...
                                                                        limit 1""",
            "Stop duration distribution by violation" :"""SELECT violation, COUNT(*) AS total_stop FROM SECURE_LIST_LOG GROUP BY violation ORDER BY total_stop DESC""" ,
            ...
            "Driver Violation Trends Based on Age and Race" :  """select driver_age,driver_race,violation,count(*) as total_violation
                                                                  ...
                                                                  (select case
                                                                   ...
                                                                   FROM SECURE_LIST_LOG) AS grouped_data
...
# Predict Logs
            ...
            with st.form("New_Log_Form"):
                ...
                country_name=st.selectbox("Country name",["Select Country","India", "Canada", "USA"])
                driver_gender=st.selectbox("Driver Gender",["M","F"])
                driver_age=st.number_input("Driver Age", min_value=16, max_value=100, value=20)
                driver_race=st.selectbox("Driver Race",["Select Race", "Asian", "Black", "White", "Hispanic", "Other"])
         ...
         # Predict Stop_Outcome
                ...
                if not filtered_data.empty:
                    ...
                    predicted_outcome = data["stop_outcome"].mode()[0]
                    predicted_violation = data["violation"].mode()[0] # Default
